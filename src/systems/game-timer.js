...
(function () {
  ...
  window.gameTimer = {
    ...
    populateScoreTable: function () {
      ...
      const totalRow = document.createElement('tr'); totalRow.className = 'total-row'; const totalColor = totalScore >= 0 ? '#00ffcc' : '#ff6b6b'; totalRow.innerHTML = `<td><strong>TOTAL</strong></td><td></td><td style="color:${totalColor}"><strong>${totalScore > 0 ? '+' : ''}${totalScore} pts</strong></td>`; tableBody.appendChild(totalRow);
    },
    ...
    populateScoreTable3D: function () {
      ...
      const fishGroups = {}; let calcTotal = 0; caughtFishes.forEach(f=>{ const key = `${f.type}_${f.isCorrect ? 'correct' : 'incorrect'}`; if(!fishGroups[key]) fishGroups[key] = { count:0, points:0, name: (f.type==='piranha'?'🐠 Piranha':'🐟 Fish') + (f.isCorrect?' ✅':' ❌'), isCorrect: f.isCorrect }; fishGroups[key].count++; fishGroups[key].points += f.points; calcTotal += f.points; }); totalScore = calcTotal;
      // headers
      const headerBg = document.createElement('a-plane'); headerBg.setAttribute('color','#00d4ff'); headerBg.setAttribute('opacity','0.2'); headerBg.setAttribute('width','1.1'); headerBg.setAttribute('height','0.08'); headerBg.setAttribute('position','0 0 -0.01'); tableContainer.appendChild(headerBg);
      const header1 = document.createElement('a-text'); header1.setAttribute('value','Fish Type'); header1.setAttribute('align','left'); header1.setAttribute('color','#00ffcc'); header1.setAttribute('width','1'); header1.setAttribute('position','-0.52 0 0'); tableContainer.appendChild(header1);
      const header2 = document.createElement('a-text'); header2.setAttribute('value','Quantity'); header2.setAttribute('align','center'); header2.setAttribute('color','#00ffcc'); header2.setAttribute('width','1'); header2.setAttribute('position','0 0 0'); tableContainer.appendChild(header2);
      const header3 = document.createElement('a-text'); header3.setAttribute('value','Points'); header3.setAttribute('align','right'); header3.setAttribute('color','#00ffcc'); header3.setAttribute('width','1'); header3.setAttribute('position','0.52 0 0'); tableContainer.appendChild(header3);
      ...
      // TOTAL
      yPosition -= 0.02; const totalBg = document.createElement('a-plane'); totalBg.setAttribute('color','#00d4ff'); totalBg.setAttribute('opacity','0.25'); totalBg.setAttribute('width','1.1'); totalBg.setAttribute('height','0.09'); totalBg.setAttribute('position',`0 ${yPosition} -0.01`); tableContainer.appendChild(totalBg); const totalLabel = document.createElement('a-text'); totalLabel.setAttribute('value','TOTAL'); totalLabel.setAttribute('align','left'); totalLabel.setAttribute('color','#00ffcc'); totalLabel.setAttribute('width','1'); totalLabel.setAttribute('position',`-0.52 ${yPosition} 0`); tableContainer.appendChild(totalLabel); const totalValue = document.createElement('a-text'); const totalColor = totalScore >= 0 ? '#00ffcc' : '#ff6b6b'; const totalSign = totalScore > 0 ? '+' : ''; totalValue.setAttribute('value', `${totalSign}${totalScore} pts`); totalValue.setAttribute('align','right'); totalValue.setAttribute('color', totalColor); totalValue.setAttribute('width','1'); totalValue.setAttribute('position', `0.52 ${yPosition} 0`); tableContainer.appendChild(totalValue);
    ...
    resetGame: function () {
      ...
      const timer3D = document.querySelector('#timer-3d'); if (timer3D) timer3D.setAttribute('visible','false');
      const timerDisplay = document.getElementById('timer-display'); if (timerDisplay) { timerDisplay.style.display = 'none'; timerDisplay.textContent = '1:00'; timerDisplay.style.color = '#00ffcc'; }
      const timerText3D = document.querySelector('#timer-text'); if (timerText3D) { timerText3D.setAttribute('value','1:00'); timerText3D.setAttribute('color','#00ffcc'); }
